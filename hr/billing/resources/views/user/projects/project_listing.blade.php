...
<div id="table_data_page">
  <h3 class="main-text-h3">Assinged Projects</h3>
      ...
      <table id="data_table">
        ...
        @foreach($userprojects as $index => $project)
          ...
          <tr>
            ...
            <td>{{ $project->title }}</td>
            <td>
            <form id="checkInForm" action="{{ route('project.tapin') }}" method="POST">
                @csrf
                <input type="hidden" name="start_time" id="startTimeInput">
                <input type="hidden" name="project_id" value="{{ $project->id }}" id="project_id">
                <button class="checkin-btn">Tap In</button>
            </form>
            </td>
            <td>

            <form id="checkoutForm" action="{{ route('project.tapout') }}" method="POST">
                @csrf
                <input type="hidden" name="total_time" id="totalTimeInput">
                <button class="checkout-btn">Tap Out</button>
            </form>
            </td>
             <td>
            @forelse($project->projectUsers as $item)
                {{ $item->formatted_time ?? '00:00:00' }}<br>
            @empty
                -
            @endforelse
        </td> 
...
function startTimer() {
            startTime = new Date(); 
            document.getElementById('startTimeInput').value = startTime.toISOString(); 

            timerInterval = setInterval(() => {
                const now = new Date();
                const diffInSeconds = Math.floor((now - startTime) / 1000);
                document.getElementById('timeDisplay').textContent = formatTime(diffInSeconds);
            }, 1000);
        }
        ...
        function stopTimer() {
            ...
            document.getElementById('totalTimeInput').value = formatTime(totalSeconds);
        }
...
function handleTableSideEdit(element) {
  ...
  const baseUpdateUrl = "{{ url('/Status/projected/updated/') }}";
