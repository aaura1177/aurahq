      ...
      <ul id="profile_notification_container">
        ...
        <li>
          <div id="user_profile" class="no-class" onclick="handleProfileIcon(this)">
                test
        <!-- <img  src="{{ session('profile_picture') ? asset('storage/' . session('profile_picture')) : asset('images/default-profile.png') }}" /> -->
...
<center id="user_name">{{ session('user_name') }}</center>
              ...
              <button id="change_password"><a href="{{ url('/admin/change-password') }}">CHANGE PASSWORD </a></button>
          </div>
          <div class="profile-features">
            <button id="profile" class="blue-bg-button"><a href="{{ url('/admin/profile') }}"style="color: white; text-decoration: none;">Profile</a></button>
            <button id="sign_out" class="blue-bg-button"><a href="{{ url('logout') }}"style="color: white; text-decoration: none;">Sign Out</a></button>
          </div>
          </div>
        </div>
      </ul>
