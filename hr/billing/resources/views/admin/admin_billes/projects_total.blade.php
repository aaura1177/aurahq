...
<div id="table_data_page">
  ...
  @foreach($data as $index => $item)
        ...
        <td>{{ $item->project->title ?? 'N/A' }}</td>
            <td>{{ $item->total_time }}</td>
        <!-- <td> {{ $item->total_minutes  ??  '00:00:00'   }}</td> -->
            <td class="action-bar">
...
<a href="{{ route('admin.view.payments', ['project_id' => $item->project_id, 'user_id' => $item->user_id]) }}">
  <li style="color: white; text-decoration: none;">View Payment Details</li>
</a>
     

        <li onclick='handleTableDelete(this)' data-id="{{ $item->id }}"><i class="fa fa-trash"></i>Delete</li>
<!-- <a href="">
  <li style="color: white; text-decoration: none;">Receiveds</li>
</a> -->

<a href="{{ route('admin.receveds', [ 'user_id' => $item->user_id ,'project_id' => $item->project_id]) }}">
  <li style="color: white; text-decoration: none;">Receiveds</li>
</a>           
         </ul>  
              </div>
...
function handleTableDelete(element) {
    ...
    form.action =`/admin/view_details/${id}`;
