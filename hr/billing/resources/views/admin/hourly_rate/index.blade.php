...
<div id="table_data_page">
  ...
  <div id="buttons_container">
    <a href="{{ route('admin.add.rate') }}"><button class="btn success"><i class="fa fa-plus"></i>Add Rate</button></a>
  ...
  <div id="data_container">
    ...
    <div id="table_container">
      <table id="data_table">
        ...
        <tbody>
            @foreach($hourlyRates as $index => $rate)
          <tr>
        <td>{{ $index + 1 }}</td> 
       <td>{{ $rate->user->name ?? 'N/A' }}</td>
         <td>{{ $rate->project->title ?? 'N/A' }}</td>      
              <td>{{ $rate->h_rate }}</td>
         <td>{{ \Carbon\Carbon::parse($rate->effective_from)->format('d M Y') }}</td>
            <td class="action-bar">
        ...
        <li onclick='handleTableDelete(this)' data-id={{ $rate->id }}><i class="fa fa-trash"></i>Delete</li>
<!-- <a href="">
