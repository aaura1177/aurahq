...
@section('title', 'New Revenue Target')
@section('header', 'New Revenue Target')

@section('content')
<div class="max-w-lg mx-auto bg-white rounded-xl border border-slate-100 shadow-sm p-6">
    <form action="{{ route('revenue-targets.store') }}" method="post" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-bold text-slate-700 mb-1">Month</label>
            <input type="month" name="month" value="{{ old('month', now()->format('Y-m')) }}" class="w-full border rounded-lg px-3 py-2" required>
        ...
        <div>
            ...
            <input type="number" step="0.01" min="0" name="target_amount" value="{{ old('target_amount') }}" class="w-full border rounded-lg px-3 py-2" required>
